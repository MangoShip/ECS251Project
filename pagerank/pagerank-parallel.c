...
void create_threads(){
    //TO DO: CREATE THREADS BASED ON GRAPH SIZE
    int i;
...
void pagerank(){
  double error = 100000;
  new_eigen = malloc(num_nodes * sizeof(double));
  while(error > threshold){
    //TO DO: PARALLIZE THIS SECTION
...
void create_matrix(){
    ...
    for (int i = 0; i < num_nodes; i++)
        matrix[i] = (double*)malloc(num_nodes * sizeof(double));
    eigen = (double *)malloc(num_nodes * sizeof(double));
    for(int i = 0; i < num_nodes; i++){
        eigen[i] = 1;
    }
