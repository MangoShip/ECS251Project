...
// Optimized merge using a temporary buffer.
...
// Structure for individual merge tasks.
typedef struct
{
    int *arr;
    int *temp;
...
// Structure to pass a batch of merge tasks to one thread.
typedef struct
{
    int *arr;
    int *temp;
    int curr_size;
    int left;       // starting index for this merge pass
    int right;      // ending index for this merge pass
    int start_task; // index of first merge task assigned to this thread
    int end_task;   // index (non-inclusive) of merge tasks for this thread
} merge_pass_args;
...
// Iterative (bottom‑up) merge sort using tholder.
void merge_sort_parallel(int *arr, int left, int right)
{
    ...
    {
        // Count number of merge tasks.
        int num_merges = 0;
        for (int i = left; i <= right - curr_size; i += 2 * curr_size)
            num_merges++;
        // If many merge tasks, partition them among a fixed set of threads.
        if (num_merges > global_desired_threads)
        {
            int num_threads = global_desired_threads;
            tholder_t *threads = malloc(num_threads * sizeof(tholder_t));
            ...
            for (int t = 0; t < num_threads; t++)
            {
                ...
                if (tholder_create(&threads[t], NULL, merge_pass_thread, margs) != 0)
                {
        ...
        {
            // If few merge tasks, spawn one thread per merge.
            int merge_count = 0;
            tholder_t *threads = malloc(num_merges * sizeof(tholder_t));
            for (int i = left; i <= right - curr_size; i += 2 * curr_size)
            {
                ...
                else
                {
                    ...
                    mp->mid = mid;
                    mp->right = r;
                    if (tholder_create(&threads[merge_count], NULL, merge_thread, mp) != 0)
                    {
                        perror("tholder_create");
                        exit(1);
                    }
